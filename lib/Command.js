...
class Command {
    ...
    validateInput(cmd, args, data) {
        ...
        const [ activity, date, time, players ] = args;

        // Input must at least have 4 parameters
        if (args.length < 4) { this.message.channel.send(`Er ontbreekt een parameter. Zie \`${this.client.config.prefix}${this.help.name} help\` voor hulp.`); return false; }

        // Validate activity
        if (!data.mode.hasOwnProperty(activity)) {
            this.message.channel.send(`\`${activity}\` is geen geldige activiteit.`); return false;
        }
    ...
    createActivityEmbed(activity, args, data) {
        ...
        let thumb = new Attachment('./data/images/badge/crucible.png', 'thumb.png');
